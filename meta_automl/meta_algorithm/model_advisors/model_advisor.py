...
class SimpleSimilarityModelAdvisor(ModelAdvisor):
    def __init__(self, fitted_similarity_assessor: DatasetsSimilarityAssessor):
        ...
        self.best_models: Dict[str, List] = {}

    @property
    def datasets(self):
        return self.similarity_assessor.datasets

    def fit(self, dataset_names_to_best_pipelines: Dict[str, List]):
        self.best_models.update(dataset_names_to_best_pipelines)
        return self

    def predict(self, meta_features: pd.DataFrame) -> List[List]:
    ...
    def _predict_single(self, similar_datasets) -> List:
        dataset_pipelines = []
        for dataset_name in similar_datasets:
